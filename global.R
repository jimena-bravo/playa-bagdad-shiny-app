...
# Carga de datos de multas, sanciones y shapes a nivel estatal y municipal -----
# Esta sección de la carga de información es la que se deberá actualizar con un 
# mecanismo una vez se tenga acceso al SIIP en tiempo real
load("datos/multas_sanciones.RData")
load("datos/shapes_mexico.RData")
load("datos/lag_madre.RData")
load("datos/playa_bagdad.RData")

load("datos/municipios.RData")
load("datos/ageb_rural.RData")
load("datos/ageb_urbano.RData")
load("datos/localidad_amanzanada.RData")
load("datos/localidad_puntual.RData")
load("datos/manzana.RData")
load("datos/vialidad.RData")
...
df_agee <- agee %>% as.data.frame()
df_agee %<>% .[-4]
df_agem <- agem %>% as.data.frame()
df_agem %<>% .[-5]

df_agee$NOMGEO %<>% homologacion_basica()
df_agem$NOMGEO %<>% homologacion_basica()
df_agem$cve_ent_nom_mun <- paste0(df_agem$CVE_ENT, " - ", df_agem$NOMGEO)

# Creación de varibles auxilaes 
sanciones <- sanciones %>%
